...
test("Missions list shows data when rendered with new missions data", () => {
    ...
    //tests for empy array
    expect(initialMissions).toHaveLength(0);
    expect(initialMissions).toEqual([]);

    //Act: re-render the component with mission data
    //this simulates when user clicks get data, triggering an api request
    rerender(<MissionsList errors="" missions={data} />);
    const missions = getAllByTestId(/mission/i);

    //Asser: 3 missions should render to the page
